		...
		try {
			if (supabaseAdmin && saveId && userId) {
				try {
					await supabaseAdmin
						.from('notion_saves')
						.update({ status: 'running', started_at: startedAt, provider: provider || null, model: model || null })
						.eq('id', saveId)
						.eq('user_id', userId);
				} catch {}
			}
				...
				try {
					...
					if (prov && mod) {
						...
						if (price) {
							...
							usage.estimated_cost_usd = inK * Number(price.input_usd_per_1k) + outK * Number(price.output_usd_per_1k);
						}
					}
				} catch {}
			}
