...
function buildImageFetchHeaders(imageUrl: string, refererUrl?: string): Record<string, string> {
  ...
  try {
    ...
    if (u.hostname.endsWith("x.com") || u.hostname.endsWith("twimg.com"))
...
async function uploadExternalImageToNotion(
  imageUrl: string,
  token: string,
  diags?: any[],
  refererUrl?: string,
  ...
  try {
    ...
    try {
      resp = await fetch(imageUrl, {
        headers: buildImageFetchHeaders(imageUrl, refererUrl),
      } as any);
      if (!resp.ok) {
      ...
      if (Array.isArray(diags))
        diags.push({ ...diagBase, step: "fetch", ok: false, error: String(e?.message || e) });
      throw e;
    }
    ...
    if (size && size > 20 * 1024 * 1024) {
      if (Array.isArray(diags))
        diags.push({ ...diagBase, step: "guard", ok: false, reason: "image too large (>20MB)" });
    ...
    if (!/\.[A-Za-z0-9]+$/.test(filename)) filename += mimeExt;
      ...
      if (mimeExt && currentExt.toLowerCase() !== mimeExt)
        filename = filename.replace(/\.[A-Za-z0-9]+$/, mimeExt);
    }
    let upInit;
    try {
      ...
      if (Array.isArray(diags))
        diags.push({
          ...diagBase,
          step: "start_file_upload",
          ok: true,
          file_upload_id: upInit?.id,
        });
    } catch (e: any) {
      if (Array.isArray(diags))
        diags.push({
    ...
    if (upInit.upload_fields && typeof upInit.upload_fields === "object") {
      ...
      if (Array.isArray(diags))
        diags.push({ ...diagBase, step: "upload", method: "POST", ok: true, status: up.status });
    } else {
      let targetHost = "";
      try {
        targetHost = new URL(upInit.upload_url).hostname;
      } catch {}
      if (targetHost.endsWith("notion.com")) {
        ...
        up = await fetch(upInit.upload_url, {
          method: "POST",
          headers: postHeaders,
          body: fd as any,
        });
        ...
        if (Array.isArray(diags))
          diags.push({ ...diagBase, step: "upload", method: "PUT", ok: true, status: up.status });
      }
    }
...
async function materializeExternalImagesInPropsAndBlocks(
  db: any,
  props: any,
  blocks: any[],
  token: string,
  ...
  for (const [propName, def] of Object.entries(db.properties || {})) {
    ...
    for (const it of items) {
      if (it?.file_upload?.id || it?.file_upload?.file_upload_id) {
        out.push(it);
        continue;
      }
      ...
      if (typeof ext === "string") {
        ...
        if (up) {
          const fid = (up as any).file_upload_id;
    ...
    for (let i = blocks.length - 1; i >= 0; i--) {
      ...
      if (blk && blk.type === "image") {
        ...
        if (!hasUpload) {
          let host = "";
          try {
            const u = new URL(blk?.image?.external?.url || "");
            host = u.hostname;
          } catch {}
...
export async function runSaveToNotion(
  payload: NotionSavePayload,
): Promise<{
  page: any;
  uploadDiagnostics: any[];
  usage?: {
    provider: string;
    model: string;
  ...
  if (!databaseId || !pageContext) throw new Error("Missing databaseId or pageContext");

  const provider: "openai" | "google" = providerIn === "google" ? "google" : "openai";
  const model: string =
    typeof modelIn === "string"
      ? modelIn
      : provider === "google"
        ? "gemini-2.5-flash"
        : "gpt-5-nano";
  ...
  if (tokens.length === 0) throw new Error("No Notion workspaces linked");

  let db: any = null;
  let notionToken: string | null = null;
  for (const tok of tokens) {
    try {
      db = await notionFetch(`/databases/${databaseId}`, {}, tok);
      notionToken = tok;
      break;
    } catch {}
  }
  if (!db || !notionToken) throw new Error("Database not accessible with linked workspaces");
  ...
  function extractSchemaForPrompt(database: any) {
    ...
    Object.entries(props).forEach(([name, def]: any) => {
      ...
      if (def.type === "select" || def.type === "multi_select")
        base.options = def[def.type]?.options?.map((o: any) => o.name) || [];
  ...
  const llmUrl = `${env.NEXT_PUBLIC_BASE_URL}/api/llm/chat`;
  const llmResp = await fetch(llmUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ provider, model, messages, reasoning_effort, verbosity }),
  });
  if (!llmResp.ok) {
  ...
  const parsed =
    ...
    (content && content.trim().startsWith("{") ? JSON.parse(content) : null);
  if (!parsed || !parsed.properties) throw new Error("LLM did not return valid properties JSON");

  const urlPropName = Object.entries(db.properties || {}).find(
    ([, def]: any) => def.type === "url",
  )?.[0];
  ...
  function sanitizeRichTextArray(arr: any[]): any[] {
    ...
    for (const it of Array.isArray(arr) ? arr : []) {
      ...
      if (it && typeof it === "object") {
        if ((it as any).type === "text" && (it as any).text?.content) {
          out.push(it);
          continue;
        }
        if (typeof (it as any).plain_text === "string") {
          out.push(make((it as any).plain_text, (it as any).href));
          continue;
        }
        if (typeof (it as any).content === "string") {
          out.push(make((it as any).content));
          continue;
        }
      }
    }
    ...
    switch (type) {
      ...
      case "rich_text": {
        if (Array.isArray(value?.rich_text))
          return { rich_text: sanitizeRichTextArray(value.rich_text) };
      ...
      case "files": {
        ...
        for (const item of arr) {
          ...
          if (typeof item === "string") {
            files.push({ name: "file", external: { url: item } });
            continue;
          }
          if (typeof item?.url === "string") {
            files.push({ name: item.name || "file", external: { url: item.url } });
            continue;
          }
          if (item.external?.url) {
            files.push({ name: item.name || "file", external: { url: item.external.url } });
            continue;
          }
        }
      ...
      case "select": {
        ...
        if (typeof name === "string" && name.trim().length > 0)
          return { select: { name: name.trim() } };
        return undefined;
      }
      case "multi_select": {
        const arr = Array.isArray(value)
          ? value
          : Array.isArray(value?.multi_select)
            ? value.multi_select
            : typeof value === "string"
              ? value.split(",")
              : undefined;
        if (Array.isArray(arr)) {
          const cleaned = arr
            .map((v: any) => (typeof v === "string" ? v.trim() : v?.name))
    ...
    for (const [propName, def] of Object.entries(db.properties || {}) as [string, any][]) {
      ...
      if (def.type === "select" && incoming.select?.name) {
        ...
        if (!existingNames.has(desired)) {
          ...
          if (capacity > 0) {
            updates[propName] = existingOpts.concat([{ name: desired, color: "default" }]);
          }
        }
      }
      if (def.type === "multi_select" && Array.isArray(incoming.multi_select)) {
        ...
        const existingNames = new Set(existingOpts.map((o: any) => o.name));
        const desiredNames = incoming.multi_select
          .map((o: any) => o.name)
        ...
        const toAdd = missing
          .slice(0, capacity)
          .map((n: string) => ({ name: n, color: "default" }));
  ...
  function sanitizeBlocks(rawBlocks: any[]): any[] {
    ...
    function normalizeRichTextArray(value: any): any[] {
      ...
      for (const it of list) {
        if (typeof it === "string") {
          out.push({ type: "text", text: { content: it } });
          continue;
        }
        if (it && typeof it === "object") {
          ...
          const fromPlain =
            typeof (it as any).plain_text === "string" ? (it as any).plain_text : undefined;
          const fromContent =
            typeof (it as any).content === "string" ? (it as any).content : undefined;
          const fromText = typeof (it as any).text === "string" ? (it as any).text : undefined;
          const chosen = fromPlain ?? fromContent ?? fromText;
          if (typeof chosen === "string") {
            out.push({ type: "text", text: { content: chosen } });
            continue;
          }
        }
      }
    ...
    for (const b of rawBlocks) {
      if (typeof b === "string") {
        out.push({
          object: "block",
          type: "paragraph",
          paragraph: { rich_text: toRichTextBlock(b) },
        });
        continue;
      }
  ...
  if (saveArticle) {
    ...
    if (
      customizeContent &&
      contentPrompt &&
      typeof pageContext?.article?.html === "string" &&
      pageContext.article.html.length
    ) {
      ...
      try {
        const tResp = await fetch(llmUrl, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            provider,
        ...
        if (tResp.ok) {
          ...
          if (!parsedBlocks) {
            ...
            if (fence) {
              try {
                parsedBlocks = JSON.parse(fence[1]);
              } catch {}
            }
          }
          if (parsedBlocks && !Array.isArray(parsedBlocks)) {
            if (Array.isArray(parsedBlocks.children)) parsedBlocks = parsedBlocks.children;
            else if (Array.isArray((parsedBlocks as any).blocks))
              parsedBlocks = (parsedBlocks as any).blocks;
            else if (Array.isArray((parsedBlocks as any).content))
              parsedBlocks = (parsedBlocks as any).content;
          }
    ...
    if (!children || !Array.isArray(children) || children.length === 0) {
      const imageOnly = (
        Array.isArray(pageContext.articleBlocks) ? sanitizeBlocks(pageContext.articleBlocks) : []
      )
        .filter((b) => b && b.type === "image")
        .slice(0, 30);
      let summaryBlocks: any[] = [];
      try {
        const contextText = String(pageContext.article?.text || pageContext.textSample || "").slice(
          0,
          4000,
        );
        const summaryMessages = [
          {
            role: "system",
            content:
              "Return only a JSON array with a single Notion paragraph block that briefly explains why this insight matters now. Use neutral, concise language (1-2 sentences).",
          },
          { role: "user", content: `Context:\n${contextText}` },
          {
            role: "user",
            content: `Additional instructions for the paragraph:\n${contentPrompt || ""}`,
          },
        ];
        const sResp = await fetch(llmUrl, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            provider,
        ...
        summaryBlocks = [
          {
            object: "block",
            type: "paragraph",
            paragraph: { rich_text: toRichTextBlock("Summary unavailable.") },
          },
        ];
  ...
  await materializeExternalImagesInPropsAndBlocks(
    db,
    safeProps,
    children,
    notionToken,
  ...
  const promptTokens =
    numberOrZero(usagePrimary?.prompt_tokens) +
    numberOrZero((globalThis as any).usageTransform?.prompt_tokens) +
    numberOrZero((globalThis as any).usageSummary?.prompt_tokens);
  const completionTokens =
    numberOrZero(usagePrimary?.completion_tokens) +
    numberOrZero((globalThis as any).usageTransform?.completion_tokens) +
    numberOrZero((globalThis as any).usageSummary?.completion_tokens);
  const totalTokens =
    numberOrZero(usagePrimary?.total_tokens) +
    numberOrZero((globalThis as any).usageTransform?.total_tokens) +
    numberOrZero((globalThis as any).usageSummary?.total_tokens);
  const callCount =
    1 +
    (typeof (globalThis as any).usageTransform === "object" ? 1 : 0) +
    (typeof (globalThis as any).usageSummary === "object" ? 1 : 0);
  const usageOut = {
    provider,
    model,
    ...
    estimated_cost_usd: undefined as number | undefined,
  };
