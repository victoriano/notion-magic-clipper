  ...
  try {
    ...
    if (active) {
      const { data, error } = await supabaseAdmin
        .from("notion_saves")
        .select(
          "id, status, run_id, notion_page_id, notion_page_url, error, started_at, completed_at, source_url, title, database_id",
        )
        .eq("user_id", userId)
    ...
    if (recent) {
      const { data, error } = await supabaseAdmin
        .from("notion_saves")
        .select(
          "id, status, notion_page_id, notion_page_url, error, started_at, completed_at, source_url, title",
        )
        .eq("user_id", userId)
        .order("started_at", { ascending: false })
        .limit(20);
    ...
    if (!id && !runId)
      return withCors(req, NextResponse.json({ error: "Missing id or run_id" }, { status: 400 }));
    let query = supabaseAdmin
      .from("notion_saves")
      .select(
        "id, user_id, status, notion_page_id, notion_page_url, error, started_at, completed_at, source_url, title",
      )
      .eq("user_id", userId)
      .limit(1);
