...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const contentPrompt: string =
      typeof body?.contentPrompt === "string" ? body.contentPrompt.trim() : "";
    const reasoning_effort: string | undefined =
      typeof body?.reasoning_effort === "string" ? body.reasoning_effort : undefined;
    const verbosity: string | undefined =
      typeof body?.verbosity === "string" ? body.verbosity : undefined;
    if (!databaseId || !pageContext)
      return withCors(
        req,
    ...
    const hasTrigger =
      Boolean(process.env.TRIGGER_SECRET_KEY) && Boolean(process.env.TRIGGER_PROJECT_ID);
    if (hasTrigger) {
      try {
    ...
    if (tokens.length === 0)
      return withCors(
        req,
        NextResponse.json({ error: "No Notion workspaces linked" }, { status: 400 }),
      );
    ...
    if (!db || !notionToken)
      return withCors(
        req,
        NextResponse.json(
          { error: "Database not accessible with linked workspaces" },
          { status: 403 },
        ),
      );
    ...
    if (!parsed || !parsed.properties)
      return withCors(
        req,
        NextResponse.json({ error: "LLM did not return valid properties JSON" }, { status: 400 }),
      );
      ...
      if (!children || !Array.isArray(children) || children.length === 0) {
        ...
        try {
          const contextText = String(
            pageContext.article?.text || pageContext.textSample || "",
          ).slice(0, 4000);
