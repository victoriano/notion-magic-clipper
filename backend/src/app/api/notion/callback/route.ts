...
export async function GET(req: NextRequest) {
	...
	try {
		...
		if (upsertRes.error) {
			return NextResponse.json({ error: 'Failed to store token', detail: upsertRes.error.message }, { status: 500 });
		}

		cookies().delete('notion_oauth_state');

		return NextResponse.redirect(`${env.NEXT_PUBLIC_BASE_URL}/connected?workspace_id=${encodeURIComponent(tokenJson.workspace_id)}`);
	} catch (error: unknown) {
