...
function collectPageContext() {
  ...
  function findNearestHeading(el) {
    ...
    for (let up = 0; up < 3 && node; up++) {
      ...
      if (node.matches && node.matches("h1,h2,h3"))
        return textTrim(node.innerText || node.textContent);
      // previous sibling or its descendants
      let sib = node.previousElementSibling;
      while (sib) {
        if (sib.matches && sib.matches("h1,h2,h3"))
          return textTrim(sib.innerText || sib.textContent);
  ...
  (function filterImagesBySize() {
    function dimOk(entry) {
      const dims = [entry.width, entry.height, entry.renderedWidth, entry.renderedHeight].filter(
