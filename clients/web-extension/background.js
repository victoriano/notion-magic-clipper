...
async function updateBadge() {
  try {
    ...
    const usingLocal =
      typeof backendUrl === "string" && /^https?:\/\/localhost(?::\d+)?/i.test(backendUrl);
    ...
    if (!isProdExtension || usingLocal) {
      try {
        await chrome.action.setBadgeBackgroundColor({ color: "#9333ea" });
      } catch {}
      try {
        await chrome.action.setBadgeText({ text: "DEV" });
      } catch {}
      try {
        await chrome.action.setTitle({ title: "Notion Magic Clipper — Dev" });
      } catch {}
    } else {
      try {
      ...
      try {
        await chrome.action.setTitle({ title: "Notion Magic Clipper" });
      } catch {}
    }
  } catch {
    try {
...
function normalizeExternalImageUrl(raw) {
  try {
    ...
    if (
      (u.hostname === "www.notion.so" || u.hostname.endsWith(".notion.so")) &&
...
async function uploadExternalImageToNotion(imageUrl) {
  try {
    ...
    dbgBg("UPLOAD: fetching external image", {
      original: redactUrl(imageUrl),
      normalized: redactUrl(normalized),
    });
    ...
    const { file_upload_id, upload_url, upload_headers, upload_fields } =
      await startFileUpload(filename);
    ...
    if (upload_fields && typeof upload_fields === "object") {
      ...
      let targetHost = "";
      try {
        targetHost = new URL(upload_url).hostname;
      } catch {}
...
async function materializeExternalImagesInPropsAndBlocks(
  ...
  for (const [propName, def] of Object.entries(db.properties || {})) {
    ...
    for (const it of items) {
      if (it?.file_upload?.file_upload_id || it?.file_upload?.id) {
        out.push(it);
        continue;
      }
      ...
      if (it?.external?.url)
        out.push({ name: it?.name || "image", external: { url: it.external.url } });
  ...
  (async () => {
    ...
    if (message?.type === "LIST_DATABASES") {
      try {
        ...
        if (message?.query ?? notionSearchQuery)
          params.set("q", String(message.query ?? notionSearchQuery));
        ...
        try {
          ...
          if (wsResp.ok) {
            ...
            wsMap = arr.reduce((acc, w) => {
              if (w?.workspace_id) acc[w.workspace_id] = w.workspace_name || "";
              return acc;
            }, {});
          }
        } catch {}
        const annotated = bases.map((d) => ({
          ...d,
          workspaceName: d.workspaceId && wsMap[d.workspaceId] ? wsMap[d.workspaceId] : "",
        }));
        // Persist cache
        try {
          await setStorage({
            dbIndexCache: { items: annotated, version: data?.version || null, ts: Date.now() },
          });
        } catch {}
    ...
    if (message?.type === "LIST_UNTITLED_DATABASES") {
      try {
        ...
        const tokensMap =
          workspaceTokens && typeof workspaceTokens === "object" ? workspaceTokens : {};
        ...
        if (tokenValues.length === 0) {
          ...
          const per = await Promise.all(
            tokenValues.map(async (tok) => {
              try {
                return await searchUntitledDatabases(tok);
              } catch {
                return [];
              }
            }),
    ...
    if (message?.type === "LIST_ALL_DATABASES") {
      try {
        ...
        if (!notionToken) throw new Error("Missing Notion token. Configure it.");
        const list = await searchAllDatabases(notionToken, {
          query: message.query ?? notionSearchQuery ?? "",
        });
        sendResponse({ ok: true, databases: list });
      } catch (e) {
        sendResponse({ ok: false, error: String(e.message || e) });
    ...
    if (message?.type === "START_FILE_UPLOAD") {
      try {
        ...
        if (!res?.file_upload_id || !res?.upload_url)
      ...
      try {
        ...
        try {
          if (Array.isArray(json?.uploadDiagnostics) && json.uploadDiagnostics.length) {
            try {
              window.lastUploadDiagnostics = json.uploadDiagnostics;
            } catch {}
            // Print full object without truncation
            console.log(
              "[NotionMagicClipper] UPLOAD_DIAGNOSTICS (full)",
              JSON.parse(JSON.stringify(json.uploadDiagnostics)),
            );
          }
        } catch {}
          ...
          try {
            ...
            try {
              ...
              await recordRecentSave({
                url: finalUrl || page?.url || page?.public_url || "",
                ts: Date.now(),
                databaseId,
            ...
            sendResponse({
              ok: true,
              page: { ...page, url: finalUrl || page?.url },
              uploadDiagnostics: json?.uploadDiagnostics || [],
              saveId,
            });
            return;
          } catch (e) {
    ...
    try {
      sendResponse({
        ok: false,
        error: `Tipo de mensaje no reconocido: ${message?.type || "desconocido"}`,
      });
    } catch (_) {}
  })();
